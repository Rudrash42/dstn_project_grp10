...
RESULTS_CSV = BASE_DIR / "experiment_results_4.csv"
PLOTS_DIR   = BASE_DIR / "plots4"
...
def estimate_l2_cpu_cache_mb(disk_cache_mb):
    """
    Estimate L2 CPU cache usage.

    LMCache writes new KV chunks to BOTH CPU (L2) and disk (L3) in
    parallel — they are **not** sequential overflow tiers.  A chunk
    that appears on disk is also present in CPU (until the CPU budget
    is exceeded and old entries are evicted).

    Since L3 disk is a direct measurement and L2 mirrors it:

        L2 ≈ min(disk_cache_mb, L2_budget)

    When disk_cache_mb < budget, L2 holds the same data as L3.
    When disk_cache_mb > budget, L2 is saturated at the budget and
    only retains the most-recent chunks.
    """
    budget_mb = MAX_CPU_CACHE_GB * 1024
    return round(min(disk_cache_mb, budget_mb), 2)
...
def run_experiment(name, prompts, llm, sp, clear_before=True, all_cold=False):
    ...
    for i, prompt in enumerate(prompts):
        ...
        # L2 mirrors L3 (parallel writes), capped by CPU budget
        l2_est_mb = estimate_l2_cpu_cache_mb(disk_after)
        l3_mb     = disk_after
